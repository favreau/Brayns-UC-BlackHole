...
# Docker container for running the BlackHole as a plugin within Brayns as a service
...
# Image where Brayns+BlackHole plugin is built
...
ARG ISPC_PATH=/app/ispc-v${ISPC_VERSION}-linux
...
# Add BlackHole
ARG BLACKHOLE_SRC=/app/blackhole
ADD . ${BLACKHOLE_SRC}
...
RUN rm -rf ${BLACKHOLE_SRC}/build \
 && mkdir -p ${BLACKHOLE_SRC}/build \
 && cd ${BLACKHOLE_SRC}/build \
 && PATH=${ISPC_PATH}/bin:${PATH} CMAKE_PREFIX_PATH=${DIST_PATH} cmake .. \
    -DCMAKE_INSTALL_PREFIX=${DIST_PATH}


RUN cd ${BLACKHOLE_SRC}/build && make -j install VERBOSE=1

# Final image, containing only Brayns and BlackHole and libraries required to run it
...
# When running `docker run -ti --rm -p 8200:8200 blackhole`,
...
CMD ["--http-server", ":8200", "--plugin", "braynsBlackHole"]
